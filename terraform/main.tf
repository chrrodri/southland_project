...
resource "aws_cloudfront_cache_policy" "html" {
  name        = "${local.name_prefix}-html-cache"
  comment     = "Short cache policy for HTML documents"
  default_ttl = 60
  max_ttl     = 300
  min_ttl     = 0
...
resource "aws_route53_record" "site" {
  count = var.create_dns_record && local.has_custom_domain && var.hosted_zone_id != "" ? 1 : 0
